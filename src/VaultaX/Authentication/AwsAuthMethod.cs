...
public sealed class AwsAuthMethod : AuthMethodBase
{
    ...
    {
        if (string.IsNullOrWhiteSpace(Options.AwsRole))
        {
            throw new VaultaXConfigurationException(
                "AwsRole is required for AWS authentication.",
                "VaultaX:Authentication:AwsRole");
        }

        var mountPath = GetMountPath("aws");
        var authType = Options.AwsAuthType?.ToLowerInvariant() ?? "iam";
        ...
        return authType switch
        {
            ...
            _ => throw new VaultaXConfigurationException(
                ...
                "VaultaX:Authentication:AwsAuthType")
        };
    }
}
